...
| [`atlas.py`](atlas.py) | Builds and maintains a knowledge map of a repo, refreshing only what changed | `FileSystem` toolsets split by scope (`filtered` read, `prefixed` write), `Planning`, `SlidingWindow`, a read-only `git_history` tool |
